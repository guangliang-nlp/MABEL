        ...
        for ratio in ratios:
            for task in tasks:
                ...
                for method in methods:
                    ...
                    for i in [1,42,100]:
                        model_file = prefix + 'test.' + task + "." + model + "." + method + ".*.ckpt"

                        for model_ in glob.glob(model_file):
                            print(model_)
                            os.system("python -m benchmark.intrinsic.stereoset.predict --model_name_or_path {0} --model {1} && python -m benchmark.intrinsic.stereoset.eval".format(model_, model))
